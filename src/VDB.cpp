...
// taken from reading online documentation and forums on how to load in VDB files

bool VDB::loadBasic()
{
    if (!m_initialised)
    {
        std::cerr<<"Unable to load data as OpenVDB has not been initialised"<<std::endl;
        return false;
    }
#ifdef EXT_GPU
    #ifdef NVIDIA
        ...
        #ifdef DEBUG
            std::cout<<"Current available GPU memory: "<<m_current_available_GPU_mem_kb/1024<<"MB"<<std::endl;
        #endif
    #else
        #ifdef DEBUG
    ...
    if (!m_fileRead)
    {
        std::cerr<<"Unable to load data as no VDB has been opened or read"<<std::endl;
        return false;
    }
...
bool VDB::loadExt()
{
    ...
    glBufferData(GL_TEXTURE_BUFFER, m_tboSize*sizeof(openvdb::Vec4f), NULL, GL_DYNAMIC_DRAW);
    // create a subbuffer data set
    glBufferSubData( GL_TEXTURE_BUFFER, 0, m_tboSize*sizeof(openvdb::Vec4f),&m_channelValueData->at(0)[0] );              // Fill
...
{
    if (m_vdbGridsInitialized) // if grids have been initilaised go ahead and remove them
    {
        for (size_t i = 0; i < m_vdbGrids->size(); i++)
        {
            m_vdbGrids->at(i).remove();
    ...
    if (m_extremesInit)
    { // if extremes have been initialised and used, clear them
        m_channelExtremes->clear();
        delete m_channelExtremes;
...
bool VDB::changeLOD()
{
    ...
    int numLoadedPoints = (int) ((float)m_numPoints.at(m_currentActiveChannelPoints)*loadFactor);
    ...
    else
    {
        ...
        m_s.at(m_currentActiveChannelPoints) = float((m_numPoints.at(m_currentActiveChannelPoints)) / (numLoadedPoints));
    }
...
void VDB::drawTree(ShaderLibrary *_shadLib)
{
    ...
    _shadLib->setShaderParam4f("loadLevels",float(m_drawTreeLevels[0]),float(m_drawTreeLevels[1]),float(m_drawTreeLevels[2]),float(m_drawTreeLevels[3]));
    m_vdbTreeVAO->bind();
    m_vdbTreeVAO->draw();
    m_vdbTreeVAO->unbind();
}
...
void VDB::drawVectors(ShaderLibrary *_shadLib)
{
    ...
    _shadLib->setShaderParam4f("u_colour",m_vectorColour.x(),m_vectorColour.y(),m_vectorColour.z(),1.0f);
...
void VDB::drawCrop(ShaderLibrary *_shadLib)
{
    for (int i = 0; i < m_numCropsToDraw; i++)
    {
        ...
        _shadLib->setShaderParam4f("u_colour",m_crop[i].colour().x(),m_crop[i].colour().y(),m_crop[i].colour().z(),1.0f);
        m_crop[i].draw();
    }
}
...
void VDB::setNumCropToDraw(int _n)
{
    ...
    {
        #ifdef DEBUG
            std::cout<<"Invalid number of crop boxes set to draw - defaulting to 1"<<std::endl;
        #endif
    }
    m_numCropsToDraw = _n;
}
...
float VDB::getS()
{
    return m_s.at(m_currentActiveChannelPoints);
}

void VDB::buildBBox(float _minx, float _maxx,
...
{
    m_crop[_index].set(_min.x(),_max.x(),_min.y(),_max.y(),_min.z(),_max.z());
    m_crop[_index].buildVAO();
}

void VDB::setCrop(float _minx, float _maxx, float _miny, float _maxy, float _minz, float _maxz, int _index)
{
    m_crop[_index].set(_minx,_maxx,_miny,_maxy,_minz,_maxz);
    m_crop[_index].buildVAO();
}

void VDB::setCrop(openvdb::Vec3f _centre, float _w, float _h, float _d, int _index)
{
...
void VDB::returnFromStorage(int _index)
{
    ...
    m_crop[_index].setwdh(m_cropStorage[_index].width(),m_cropStorage[_index].height(),m_cropStorage[_index].depth());
...
void VDB::printFileInformation()
{
    ...
    // now print our for each grid found its name, its dimensions and information stored within it
    int index = 0;
    for (openvdb::GridPtrVec::const_iterator it = m_grid->begin(); it != m_grid->end(); ++it)
    {
        if (openvdb::GridBase::ConstPtr g = *it)
        {
               std::cout << "Grid Name: " << m_gridNames->at(index) << std::endl;
               std::cout << "Voxel Dimensions: " << m_gridDims->at(index)[0] << " by " << m_gridDims->at(index)[1] << " by "
...
// logic of this function taken from studying The GL viewer provided with the OpenVDB library
...
void VDB::getMeshValuesScalar(typename GridType::ConstPtr _grid)
{
    ...
    for (typename GridType::ValueOnCIter it = _grid->cbeginValueOn(); it; ++it)
    {
        ...
        // however this could then cause it to miss out a few hundred thousand points
        // instead it will over compensate and draw a couple hundred thousand more
        // prevents a model with a hole in it
        // all caused by rounding issues
        ...
        // check for minimum
        if (point.nx < channelExtremes.minx || point.ny < channelExtremes.miny || point.nz < channelExtremes.minz)
        {
        ...
        // check for maximum
        if (point.nx > channelExtremes.maxx || point.ny > channelExtremes.maxy || point.nz > channelExtremes.maxz)
        {
...
// logic of this function taken from studying The GL viewer provided with the OpenVDB library
// this function is idnetical to the one above except it handles vector types so has a few differences which are hihglighted
template <typename GridType>
void VDB::getMeshValuesVector(typename GridType::ConstPtr _grid)
{
    ...
    openvdb::tree::ValueAccessor<const TreeType> acc(tree); // accessor into grid of the right type
...
void VDB::getTreeValues(typename GridType::Ptr _grid)
{
    ...
    {
        m_totalVoxels += m_levelCounts[i]; // calculatye the total voxels in the tree
    }
    ...
    static const GLuint elementsBare[24] = {0,1,
                                            1,2,
                                            2,3,
                                            3,0,
                                            4,5,
    ...
    for (typename GridType::TreeType::NodeCIter it = _grid->tree(); it; ++it)
    {
        ...
        min = _grid->indexToWorld(area.min().asVec3s()); // minus 0.5 off to prevent gaps in the voxels
        min[0] -= 0.5;
        min[1] -= 0.5;
        min[2] -= 0.5;
        max = _grid->indexToWorld(area.max().asVec3s()); // add 0.5 on to prevent gaps in the voxels
        max [0] += 0.5;
        max [1] += 0.5;
        max [2] += 0.5;
        ...
        pointVDat.nx = colour.x(); // store colour for this voxel level from pre defined function
    ...
    m_vdbTreeVAO->bind();
    m_vdbTreeVAO->setIndexedData(totalVertices*sizeof(vDat),vertices[0].u,totalElements,&indexes[0],GL_UNSIGNED_INT);
...
bool VDB::loadMesh()
{
    ...
    // now to actually load in the values - for now to test I will load in the colours to the normals vector
    // I have a feeling that in the future I may need to have a couple of VAO's depending on what elements I am
    // drawing or at least some more vectors somewhere to store some data
    // for now I will only load in the positions and colours
...
{
    std::cerr<<"File contain grid of type std::string - std::string is currently not supported, closing application"<<std::endl;
    exit(EXIT_FAILURE);
}
...
// process the type of grid being passed to it using templates and then call the correct function for scalar or vector to get high res data values
void VDB::processTypedGrid(openvdb::GridBase::Ptr grid)
{
    // scalar types
    if (grid->isType<openvdb::BoolGrid>())        callGetValuesGridScalar<openvdb::BoolGrid>(openvdb::gridPtrCast<openvdb::BoolGrid>(grid));
    else if (grid->isType<openvdb::FloatGrid>())  callGetValuesGridScalar<openvdb::FloatGrid>(openvdb::gridPtrCast<openvdb::FloatGrid>(grid));
    else if (grid->isType<openvdb::DoubleGrid>()) callGetValuesGridScalar<openvdb::DoubleGrid>(openvdb::gridPtrCast<openvdb::DoubleGrid>(grid));
    else if (grid->isType<openvdb::Int32Grid>())  callGetValuesGridScalar<openvdb::Int32Grid>(openvdb::gridPtrCast<openvdb::Int32Grid>(grid));
    else if (grid->isType<openvdb::Int64Grid>())  callGetValuesGridScalar<openvdb::Int64Grid>(openvdb::gridPtrCast<openvdb::Int64Grid>(grid));
    // vector types
    else if (grid->isType<openvdb::Vec3IGrid>())  callGetValuesGridVector<openvdb::Vec3IGrid>(openvdb::gridPtrCast<openvdb::Vec3IGrid>(grid));
...
{
    // scalar types
    if (grid->isType<openvdb::BoolGrid>())        getTreeValues<openvdb::BoolGrid>(openvdb::gridPtrCast<openvdb::BoolGrid>(grid));
    else if (grid->isType<openvdb::FloatGrid>())  getTreeValues<openvdb::FloatGrid>(openvdb::gridPtrCast<openvdb::FloatGrid>(grid));
    else if (grid->isType<openvdb::DoubleGrid>()) getTreeValues<openvdb::DoubleGrid>(openvdb::gridPtrCast<openvdb::DoubleGrid>(grid));
    else if (grid->isType<openvdb::Int32Grid>())  getTreeValues<openvdb::Int32Grid>(openvdb::gridPtrCast<openvdb::Int32Grid>(grid));
    else if (grid->isType<openvdb::Int64Grid>())  getTreeValues<openvdb::Int64Grid>(openvdb::gridPtrCast<openvdb::Int64Grid>(grid));
    // vector types
    else if (grid->isType<openvdb::Vec3IGrid>())  getTreeValues<openvdb::Vec3IGrid>(openvdb::gridPtrCast<openvdb::Vec3IGrid>(grid));
