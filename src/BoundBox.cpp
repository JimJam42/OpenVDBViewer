...
BoundBox::BoundBox(float _minX, float _maxX,
...
BoundBox::BoundBox(float _width, float _height, float _depth, openvdb::Vec3f _centre)
{
    init();
...
void BoundBox::set(float _minx, float _maxx, float _miny, float _maxy, float _minz, float _maxz)
{
    // set the min and max attributes and then recalculate all attributes for the bounding box
    m_minX = _minx;
    m_maxX = _maxx;
    m_minY = _miny;
    m_maxY = _maxy;
...
{
#ifdef DEBUG
    std::cout<<"Warning: Centre of bounding box being set with no other update, could cause erronous results"<<std::endl;
#endif
    m_centre[0] = _x;
    m_centre[1] = _y;
    m_centre[2] = _z;
...
void BoundBox::setwdh(float _a)
{
    ...
    m_height = _a;
    m_depth = _a;
    // setting width, depth and height. As they have all changed, need to recalculate
    // vertices from unchanged attributes
    vertsFromCentre();
}
...
void BoundBox::drawMode(GLenum _mode)
{
    ...
    if (m_buildIndexed)
    {
        // if the bounding box has been set to be built as indexed, call index function
        buildVAOIndexed();
    }
    else
    {
...
{
    // these will be ordered in a specific way. Will work around from bottom left (so minX, minY, maxZ)
    // box in a counter clockwise direction and then move to the other edge
    /*
...
void BoundBox::calculateNormals()
{
    ...
    // calculate it at the point of collision just calculate whenever the bounding box is updated
    /*
...
void BoundBox::buildVAO()
{
    ...
    // now set each one to vDat format - done manually as quicker and easier if a bit longer
...
void BoundBox::buildVAOIndexed()
{
    ...
    static const GLuint elements[24] = {0,1,
                                        1,2,
                                        2,3,
                                        3,0,
                                        4,5,
    ...
    m_vao->bind();
    m_vao->setIndexedData(buffSize*sizeof(vDat),data[0].u,elementSize*sizeof(GLuint),&elements[0],GL_UNSIGNED_INT);
    m_vao->vertexAttribPointer(0,3,GL_FLOAT,sizeof(vDat),5);
...
{
    // simple bounding box collision detection to see if the passed in point is within the box
