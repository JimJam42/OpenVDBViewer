...
/// @file BoundBox.h
/// @brief Class to create and control a bounding box. Handles its drawing and update of its properties.
/// @author Callum James
/// @version 1.0
/// @date 12/02/2014
/// Revision History:
...
public:
    ...
    BoundBox(float _minx, float _maxx,
             float _miny, float _maxy,
             float _minz, float _maxz);
    /// @brief Destructor for BoundBox
    ~BoundBox();

    /// @brief Method to return the centre of the bounding box - returns openvdb::Vec3f
    ...
    /// @brief Method to return the colour of the bounding box - returns openvdb::Vec3f
    inline openvdb::Vec3f colour() {return m_colour;}
    /// @brief Method to return if the bounding boxed has been built indexed or not - returns true or false
    inline bool getBuildIndexed() {return m_buildIndexed;}

    /// @brief Method to return the vertex at the specified index - returns openvdb::Vec3f
    ...
    /// @param [in] _minz float - minimum z value for the bounding box
    /// @param [in] _maxz float - maximum z value for the bounding box
    void set(float _minx, float _maxx,
             float _miny, float _maxy,
             float _minz, float _maxz);
    ...
    /// @param [in] _a float - value to set width, float and height of the bounding box to
    ...
    /// @brief Method to set whether or not the bounding box is to be built indexed or not
    ...
    /// @brief Method to calculate the vertices from the centre of the bounding box
    ...
    /// @brief Method to scale the bounding box
    /// @param [in] _s const openvdb::Vec3f - scale box by this vector in the corresponding axis
    ...
    /// @param [in] _build bool - boolean of whether to build the VAO of the box or not
    ...
    /// @brief Method to calculate if a point lies within the bounding box - returns true or false
    /// @param [in] _point openvdb::Vec3f - the point to test against
    bool pointInside(openvdb::Vec3f _point);

    /// @brief Return this BoundingBox as core components in the form of a BBoxBare
    BBoxBare asBBoxBare();
...
private:
    ...
    /// @brief Method to convert an openvdb::Vec3f into vDat format, writing the result to the passed in pointer
