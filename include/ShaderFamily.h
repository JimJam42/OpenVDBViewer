...
/// @file ShaderFamily.h
/// @brief ShaderFamily class represents one entire shader program and all contained shaders
/// @author Callum James
/// @version 1.0
/// @date 12/02/2014
/// Revision History:
...
/// @class ShaderFamily
/// @brief ShaderFamily class handles one shader program. It controls the linking and setting of
/// shader attributes and uniforms within the family and allows an easy encapsulation for the
/// creation of shaders as well as using them throughout the application
class ShaderFamily
{
public:
    ...
    // run through the attached shaders and compile any ones that have not been compiled
    ...
    /// @brief Method to get a uniform location within the attached shaders - returns GLuint
    /// @param [in] _name const char* - the name of the uniform to find
    GLuint getUniformLocation( const char* _name);

    // function to allow manual enabling of the vertex attrib array with the passed in id
    ...
    // function to allow manual disabling of the vertex attrib array with the passed in id
    ...
    void setUniform4f(const char* _var, float _f0, float _f1, float _f2, float _f3);
    ...
    void setUniform4i(const char* _var, GLint _i0, GLint _i1, GLint _i2, GLint _i3);
    ...
    void setUniformMatrix2fv(const char* _var, size_t _count,  bool _transpose,  const float* _m);
    /// @brief Upload a 3x3 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix3fv(const char* _var, size_t _count, bool _transpose, const float* _m);
    /// @brief Upload a 4x4 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix4fv(const char* _var, size_t _count, bool _transpose, const float* _m);
    /// @brief Upload a 2x3 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix2x3fv(const char* _var, size_t _count, bool _transpose, const float* _m);
    /// @brief Upload a 2x4 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix2x4fv(const char* _var, size_t _count, bool _transpose,  const float* _m);
    /// @brief Upload a 3x2 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix3x2fv(const char* _var, size_t _count, bool _transpose, const float* _m);
    /// @brief Upload a 3x4 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix3x4fv(const char* _var, size_t _count, bool _transpose, const float* _m);
    /// @brief Upload a 4x2 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix4x2fv(const char* _var, size_t _count, bool _transpose,  const float* _m);
    /// @brief Upload a 4x3 matrix to the shaders
    /// @param [in] _var const char* - uniform name
    /// @param [in] _count size_t - size of data
    /// @param [in] _transpose bool - transposed matrix
    /// @param [in] _m const float* - data to upload
    void setUniformMatrix4x3fv(const char* _var, size_t _count, bool _transpose, const float* _m);
